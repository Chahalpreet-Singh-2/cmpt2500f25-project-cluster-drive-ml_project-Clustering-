...
def load_data(path: str | Path) -> pd.DataFrame:
    ...
    - Loads all columns as strings (dtype=str) to avoid unwanted type inference.
    """
    p = Path(path)
    ...
    # Important: load everything as strings so tests expecting "1" (str) pass
...
def main() -> None:
    ...
    # Example if you combine 4 CSVs:
    csv_paths = [
        raw_dir / "CBB_Listings_1.csv",
        raw_dir / "CBB_Listings_2.csv",
        raw_dir / "CBB_Listings_3.csv",
        raw_dir / "CBB_Listings_4.csv",
    ]

    dfs = [load_data(p) for p in csv_paths]
    df = pd.concat(dfs, ignore_index=True)

    # ... your cleaning & feature engineering here ...

    out_parquet = root / "data" / "processed" / "final_features.parquet"
    out_parquet.parent.mkdir(parents=True, exist_ok=True)
    df.to_parquet(out_parquet, index=False)
    log.info(f"✅ Wrote {len(df):,} rows → {out_parquet}")
